...
class DataTypeParser {
    ...
    static DataType parse(String toParse, Cluster cluster, String currentKeyspaceName, Map<String, UserType> userTypes, boolean frozen) {
        ...
        if (type.equalsIgnoreCase(LIST)) {
            ...
            DataType elementType = parse(parameters.get(0), cluster, currentKeyspaceName, userTypes, false);
        ...
        if (type.equalsIgnoreCase(MAP)) {
            ...
            DataType keyType = parse(parameters.get(0), cluster, currentKeyspaceName, userTypes, false);
            DataType valueType = parse(parameters.get(1), cluster, currentKeyspaceName, userTypes, false);
        ...
        if (type.equalsIgnoreCase(FROZEN)) {
            ...
            return parse(parameters.get(0), cluster, currentKeyspaceName, userTypes, true);
        }
        ...
        if (type.equalsIgnoreCase(TUPLE)) {
            ...
            for (String rawType : rawTypes) {
                types.add(parse(rawType, cluster, currentKeyspaceName, userTypes, false));
            }
        ...
        // should only happen when we do a partial schema refresh, and not all UDTs are present in userTypes map
        return new UnresolvedUserType(currentKeyspaceName, Metadata.handleId(type), cluster);
